...
class MKV_GPIO : public halina::GPIO {
private:
    PORT_Type* port_base;
    GPIO_Type* base;
    uint32_t pin;
...
public:
    MKV_GPIO(PORT_Type* port_base, GPIO_Type *base, uint32_t pin, halina::GPIO::Mode mode = halina::GPIO::Mode::OUTPUT, port_interrupt_t configInterrupt = kPORT_InterruptOrDMADisabled, void (*callbackHandler)() = nullptr):
            port_base(port_base),
            base(base),
            pin(pin),
            mode(mode),
    ...
    void appendInterrupt(Interrupt *portInterrupts, MKV_GPIO* gpio) {
...
public:
    ...
    MKV_PORT(PORT_Type* port, uint8_t pinNumber, uint8_t muxNumber, Pull pull = Pull::Disable, OpenDrain openDrain = OpenDrain::Disable, SlewRate slewRate = SlewRate::Fast )
    ...
    static MKV_PORT& getEmptyPort(){
        static MKV_PORT port = {nullptr, 0, 0};
        return port;
    }
