 ...
 *
 * MKV HALina implementation of adc
 *
 */
...
MKV_ADC* HSADCInstances[2];

MKV_ADC::MKV_ADC(HSADC_Type* baseAdc, void (*converterAHandler)(uint8_t), void (*converterBHandler)(uint8_t)) : baseAdc(baseAdc) {
...
void MKV_ADC::Converter::init(HSADC_Type* baseAdc)  {
...
bool MKV_ADC::Converter::appendSample(Sample *sample) {
...
bool MKV_ADC::appendSample(Sample *sample) {
...
void MKV_ADC::saveData(Converter::Type converterType) {
...
uint16_t* MKV_ADC::getBufferValues(Converter::Type converterType){
    if (converterType == Converter::Type::A) {
...
void HSADC0_CCA_IRQHandler() {
    ...
    HSADCInstances[0]->saveData(MKV_ADC::Converter::Type::A);
}

void HSADC0_CCB_IRQHandler() {
    HSADC0->STAT |= HSADC_STAT_EOSIB_MASK;
    HSADCInstances[0]->saveData(MKV_ADC::Converter::Type::B);
}

void HSADC1_CCA_IRQHandler() {
    HSADC0->STAT |= HSADC_STAT_EOSIB_MASK;
    HSADCInstances[1]->saveData(MKV_ADC::Converter::Type::A);
}

void HSADC1_CCB_IRQHandler() {
    HSADC0->STAT |= HSADC_STAT_EOSIB_MASK;
    HSADCInstances[1]->saveData(MKV_ADC::Converter::Type::B);
}
}

void MKV_ADC::Sample::init(HSADC_Type* baseAdc, uint8_t indexInRegisterSample) {
    if (!initialised) {
        pinMux.setMux();
        initialised = true;
    }
