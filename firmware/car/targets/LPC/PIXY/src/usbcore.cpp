...
void USB_DataInStage () {
    ...
    cnt = USB_WriteEP(0x80u, EP0Data.pData, cnt);
    EP0Data.pData += cnt;
    EP0Data.Count -= cnt;
}
...
uint32_t USB_ReqGetStatus () {
    ...
    switch (SetupPacket.bmRequestType.BM.Recipient) {
        ...
        case REQUEST_TO_ENDPOINT:
            n = SetupPacket.wIndex.WB.L & 0x8Fu;
            m = (n & 0x80u) ? ((1u << 16u) << (n & 0x0Fu)) : (1u << n);
            if (((USB_Configuration != 0) || ((n & 0x0Fu) == 0)) && (USB_EndPointMask & m)) {
...
uint32_t USB_ReqSetAddress () {
    if (SetupPacket.bmRequestType.BM.Recipient == REQUEST_TO_DEVICE) {
        USB_DeviceAddress = 0x80u | SetupPacket.wValue.WB.L;
        return true;
    }
    return false;
}
...
uint32_t USB_ReqGetConfiguration () {
    if (SetupPacket.bmRequestType.BM.Recipient == REQUEST_TO_DEVICE) {
...
uint32_t USB_ReqSetConfiguration () {
                ...
                while (pD->bLength) {
                    switch (pD->bDescriptorType) {
                        case USB_CONFIGURATION_DESCRIPTOR_TYPE:
                            if (((USB_CONFIGURATION_DESCRIPTOR *)pD)->bConfigurationValue == SetupPacket.wValue.WB.L) {
                                ...
                                for (n = 1; n < USB_EP_NUM; n++) {
                                    if (USB_EndPointMask & (1u << n)) {
                                        USB_DisableEP(n);
                                    }
                                    if (USB_EndPointMask & ((1u << 16u) << n)) {
                                        USB_DisableEP(n | 0x80u);
                                    }
                                }
                                USB_EndPointMask = 0x00010001;
                                USB_EndPointHalt = 0x00000000;
                        ...
                        case USB_ENDPOINT_DESCRIPTOR_TYPE:
                            if (alt == 0) {
                                n = ((USB_ENDPOINT_DESCRIPTOR *)pD)->bEndpointAddress & 0x8Fu;
                                m = (n & 0x80u) ? ((1u<< 16u) << (n & 0x0Fu)) : (1u << n);
                                USB_EndPointMask |= m;
...
//                    case AUDIO_REQUEST_SET_RES:
        ...
        case USB_EVT_IN :
            if (SetupPacket.bmRequestType.BM.Dir == REQUEST_DEVICE_TO_HOST) {
                ...
                if (USB_DeviceAddress & 0x80u) {
                    USB_DeviceAddress &= 0x7Fu;
        ...
        case USB_EVT_OUT_STALL:
            USB_ClrStallEP(0x00u);
            break;

        case USB_EVT_IN_STALL:
            USB_ClrStallEP(0x80u);
            break;
