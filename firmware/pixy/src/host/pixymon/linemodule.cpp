...
void LineModule::renderLISG(uint8_t renderFlags, uint16_t width, uint16_t height, uint32_t len, uint8_t *data)
{
    ...
    for (i=0; i<len; i++)
    {
        ...
        Renderer::drawText(&m_painter, (segs[i].m_p0.m_x + segs[i].m_p1.m_x)*scale/2, (segs[i].m_p0.m_y + segs[i].m_p1.m_y)*scale/2, QString::number(i) + "," + QString::number(segs[i].m_line));
    }
...
{
    static QString outfile_name = "0000.ppm"; 
    ...
    outfile_name[0] = QChar(hex_table[(index & 0xf000)>>12]);
    outfile_name[1] = QChar(hex_table[(index & 0x0f00)>>8]);
    outfile_name[2] = QChar(hex_table[(index & 0x00f0)>>4]);
    outfile_name[3] = QChar(hex_table[(index & 0x000f)]);
    img2.save(outfile_name, "PPM");
    ++index;
}
