...
#define PIXY_TYPE_REQUEST_OPT                0x1a
#define PIXY_TYPE_RESPONSE_OPT               0x1b
...
template <class LinkType> int8_t TPixy2<LinkType>::getLines()
{
  m_length = 0;
  m_type = PIXY_TYPE_REQUEST_OPT;
  sendPacket();
  if (recvPacket()==0)
  {   
    if (m_type==PIXY_TYPE_RESPONSE_OPT)
    {
        day = *(uint8_t *)(m_buf + 0);
        month = *(uint8_t *)(m_buf + 1);
        year = *(uint16_t *)(m_buf + 2);
      return m_length;
    }
    else if (m_type==PIXY_TYPE_RESPONSE_ERROR)
      return PIXY_RESULT_BUSY;
