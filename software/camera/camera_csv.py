...
def read_csv_data():
    ...
    global data, all_data, currentRow, brightness_data, servo_data
    try:
        with open(routeFile, 'r') as csvfile:
            ...
            if rows:
                if currentRow < len(rows):
                    ...
                    brightness_data = float(latest_row[128]) * 2
                    servo_data = latest_row[129]
...
def update_plot():
    ...
    if data is not None:
        ...
        if servo_data is not None:
            servo_line.setPos(servo_data)  # Aktualizacja pozycji linii
        
        if brightness_data is not None:
            brightness_line.setPos(brightness_data)  # Aktualizacja pozycji linii
